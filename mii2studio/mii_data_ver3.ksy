...
seq:
  - id: format_version
    type: u1
    doc: Notes whether the Mii was made normally, or through the Wii U Mii Maker's camera functionality. 0 = initially created using the Wii U Mii Maker's camera, 3 = initially created otherwise
  - id: copying
    type: b1le
  ...
  - id: region_lock
    type: b2le
    enum: region
    doc: Determines if a Special Mii's QR code can only be scanned on devices of a certain region. 0 = no lock, 1 = JPN, 2 = USA, 3 = PAL/AUS. Other regions (CHN, KOR, TWN) simply must use no region lock to work. Default: 0.
  - id: font_region
    type: b2le
    enum: region_text
    doc: The font region for the Mii name and creator name. For Wii U/3DS, viewing text of a font region different from the device's region will result in the text being replaced by "???". 0 = USA + PAL + AUS + JPN, 1 = CHN, 2 = KOR, 3 = TWN.
  - id: unused_00
    type: b2le
  ...
  - id: birth_platform
    ...
    doc: The device in which the Mii was originally made on. 1 = Wii, 2 = DS, 3 = 3DS, 4 = Wii U/Switch. Setting this value to anything else will result in an invalid Mii. If is_normal in the Mii ID is set to 0, then this also determines which device the QR code can be scanned on (ex: a Special Mii with original_creation_device set to 3 can not be scanned on a Wii U). When scanning a QR code, if this value is higher than the device you're scanning it on (such as a 4 on a 3DS), then it will not scan. Since this only goes up to 4 and Wii/DS never supported QR codes, setting this to 4 in a QR code results in a Wii U-locked Mii, but a 3DS-locked Mii is not possible.
  - id: unused_01
    type: b1le
  ...
  - id: console_mac
    size: 6
    doc: Six bytes representing the full MAC address of the device the Mii was created on. The Mii ID followed by the console MAC makes up the full 10 bytes of the Create ID.
  - id: unused_02
    type: u2
    doc: These bytes are unused. They should be set to 0.
  - id: sex
    type: b1le
    enum: sexes
  ...
  - id: hair
    type: hair_data
    doc: The information relating to the Mii's hair: hair type, color, whether or not the hair is flipped, and four unused bits.
  - id: eyes
    type: eye_data
  ...
  - id: checksum
    type: u2
    doc: The Mii data file from 0x00 to 0x5C in a crc16 checksum. NOTE: This only applies to (C/F)FSD files; (C/F)FCD files do not contain these two bytes.
types:
  mii_id_contents:
    seq:
      - id: is_normal
        type: b1be
        doc: Determines if the Mii is Special (golden pants) or not. Special Miis are meant to only be created and distributed by Nintendo. 0 = Special, 1 = not Special (Normal). Also look at original_creation_device and head_data.not_sharing if the Mii is Special.
      - id: is_ds
        type: b1be
        doc: This bit is only set if the Mii was originally created on a DS system (meaning birth_platform gets set to 2), and if is_valid is set to 0 (meaning the Mii is invalid). Its purpose is still unknown.
      - id: is_developer_mii
        type: b1be
  ...
  head_data:
    seq:
      - id: not_sharing
        type: b1le
        doc: Disable Sharing. Enabling this allows the Mii to travel to others' systems via StreetPass. 0 = Sharing enabled, 1 = Sharing disabled. Default: 1. If the Mii is Special and this value is set to 0 (enabled), then the Mii will be considered invalid. Thus, all Special Miis must have Sharing disabled.
      - id: face_type
        type: b4le
